...
from tkinter import Label, StringVar, Toplevel, ttk, filedialog, messagebox
...
class MainApp(tk.Frame):
    ...
    def new_profile(self):
        # Added defaultextension parameter
        ...
        if filename != None:
            ...
            self._current_profile = NaClProfile(SERVER, USERNAME, PW)            
            self._current_profile.generate_keypair()
            #print("new dsu keypair: " + self._current_profile.keypair)
            self.body.reset_ui()
    ...
    '''
    Open a Settings window for entering username and password and saving it.
    '''
    def open_settings(self):

        # Don't allow settings to open unless user has opened or created a profile first
        if self._profile_filename == None:
            tk.messagebox.showerror("Profile not open", "You must first open or create a profile.")
            return

        self._settingsWindow = Toplevel(main)
        self._settingsWindow.title = 'Settings'
        self._settingsWindow.geometry("200x100")

        # set up the input field for username
        usernameLabel = Label(self._settingsWindow, text = "Username: ").grid(row=0,column=0)
        self._usernameSetting = StringVar()
        usernameInput = tk.Entry(self._settingsWindow, textvariable=self._usernameSetting).grid(row=0,column=1)
        self._usernameSetting.set(self._current_profile.username)

        # set up the input field for password
        passwordLabel = Label(self._settingsWindow, text = "Password: ").grid(row=1,column=0)
        self._passwordSetting = StringVar()
        passwordInput = tk.Entry(self._settingsWindow, textvariable=self._passwordSetting).grid(row=1,column=1)
        self._passwordSetting.set(self._current_profile.password)

        tk.Button(self._settingsWindow,text="Save",command = self.save_settings).grid(row=2,column=1)
        self._settingsWindow.update()
        self._settingsWindow.minsize(self._settingsWindow.winfo_width(), self._settingsWindow.winfo_height())

    '''
    Save the settings to the profile that is open
    '''
    def save_settings(self):
        self._current_profile.username = self._usernameSetting.get()
        self._current_profile.password = self._passwordSetting.get()
        self._current_profile.save_profile(self._profile_filename)
        # close settings window
        self._settingsWindow.destroy()

    '''
    Open a Bio window for entering a bio and saving it.
    '''
    def open_bio(self):

        # Don't allow bio to open unless user has opened or created a profile first
        if self._profile_filename == None:
            tk.messagebox.showerror("Profile not open", "You must first open or create a profile.")
            return

        self._bioWindow = Toplevel(main)
        self._bioWindow.title = 'Bio'
        self._bioWindow.geometry("200x100")

        # set up the input field for Bio
        bioLabel = Label(self._bioWindow, text = "Bio: ").grid(row=0,column=0)
        self._bio = StringVar()
        bioInput = tk.Entry(self._bioWindow, textvariable=self._bio).grid(row=0,column=1)
        self._bio.set(self._current_profile.bio)

        tk.Button(self._bioWindow,text="Save",command = self.save_bio).grid(row=2,column=1)
        self._bioWindow.update()
        self._bioWindow.minsize(self._bioWindow.winfo_width(), self._bioWindow.winfo_height())

    '''
    Save the bio to the profile that is open
    '''
    def save_bio(self):
        self._current_profile.bio = self._bio.get()
        self._current_profile.save_profile(self._profile_filename)
        # close bio window
        self._bioWindow.destroy()

    '''
    Open a Server window for entering a server and saving it.
    '''
    def open_server(self):

        # Don't allow server to open unless user has opened or created a profile first
        if self._profile_filename == None:
            tk.messagebox.showerror("Profile not open", "You must first open or create a profile.")
            return

        self._serverWindow = Toplevel(main)
        self._serverWindow.title = 'Server'
        self._serverWindow.geometry("200x100")

        # set up the input field for Server
        serverLabel = Label(self._serverWindow, text = "Bio: ").grid(row=0,column=0)
        self._server = StringVar()
        serverInput = tk.Entry(self._serverWindow, textvariable=self._server).grid(row=0,column=1)
        self._server.set(self._current_profile.dsuserver)

        tk.Button(self._serverWindow,text="Save",command = self.save_server).grid(row=2,column=1)
        self._serverWindow.update()
        self._serverWindow.minsize(self._serverWindow.winfo_width(), self._serverWindow.winfo_height())

    '''
    Save the server to the profile that is open
    '''
    def save_server(self):
        self._current_profile.dsuserver = self._server.get()
        self._current_profile.save_profile(self._profile_filename)
        # close server window
        self._serverWindow.destroy()
    ...
    def save_profile(self):
        # Perform whatever operations are necessary to save a 
        ...
        # Add the post to the profile
        self._current_profile.add_post(post)
        self._current_profile.save_profile(self._profile_filename)
        if self._is_online is True:
            self.publish(Post(self.body.get_text_entry()))
    ...
    def publish(self, post:Post):
        ds_client.send(PORT, post.get_entry(), self._current_profile)
    ...
    def _draw(self):
        ...
        menu_file.add_command(label='Server', command=self.open_server)
